...
description: Consolidate duplicate Zotero records BEFORE running Zotero's native "Merge Items", so the merge is lossless. Zotero's merge keeps only the master record's field values (silently dropping metadata the other copies had) and only groups items of the SAME item type — so cross-type duplicates (preprint vs journalArticle vs conferencePaper) never even show up as duplicates. This skill fixes both: given a title (or explicit item keys) it finds the duplicate records, confirms they're the same work, UNIONS their metadata (authors, abstract, DOI, URL, venue, date, extra — filling gaps from OpenAlex), and NORMALIZES their item types to one type (tagging orig-type:/orig-date: for lineage). After it runs, every duplicate has identical complete metadata + the same type, so Zotero's Duplicate Items merge drops nothing. Use whenever the user says "prep duplicates before merging", "Zotero dedupe isn't detecting these", "consolidate these duplicate records", "these two entries are the same paper", "copy metadata from one to the other", "fix the metadata before I dedupe", or is about to run Zotero's merge on records that have missing authors/DOIs or different item types. Complements the `zotero` skill (library queries) and `openalex` skill (metadata backfill).
---
