...
All operations go through `scripts/zotero.py`. **Invoke it with `python3` (not `python`)** — the `python` alias is not on this machine's PATH. Run `python3 scripts/zotero.py --help` for the full surface; the main subcommands are:
